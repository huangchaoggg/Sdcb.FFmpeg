...
namespace MediaPlayer.MediaFramework
{
    public class MediaContainer:IDisposable
    {
        ...
        public unsafe Task OpenAsync(string uri)
        {
            Uri = uri;
            try
            {
            ...
            while (true)
            {
                ...
                if (packet != null)
                {
                    foreach (var f in VideoDecoder!.DecodePacket(packet))
                    {
                        ...
                        else if ((isNext && (t - timestamp) > interval) || (!isNext && t>timestamp ))
                        {
                            return FindVideoFrame(timestamp,isNext,retry+1);
                        }
